...
public class GameManager : MonoBehaviour {
    ...
    {
        int stageIndex = (int)_stage - 1; 

        if (stageIndex >= 0 && stageIndex < _skyboxMaterials.Count && _skyboxMaterials[stageIndex] != null)
        {
            RenderSettings.skybox = _skyboxMaterials[stageIndex];
    ...
    {
        if (_skyboxMaterials.Count > 0 && _skyboxMaterials[0] != null)
            RenderSettings.skybox = _skyboxMaterials[0];
        else if (_skyboxColors.Count > 0)
